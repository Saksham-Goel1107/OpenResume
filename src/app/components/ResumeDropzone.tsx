...
// Dynamically import the PDF parser to avoid SSR issues
import dynamic from "next/dynamic";
// Import type only to avoid including the module in SSR
import type { ParseResumeFromPdfType } from "lib/parse-resume-from-pdf/types";
import {
  getHasUsedAppBefore,
...
export const ResumeDropzone = ({
  ...
  const onImportClick = async () => {
    try {
      // Dynamically import the PDF parser module
      const { parseResumeFromPdf } = await import("lib/parse-resume-from-pdf");
      const resume = await parseResumeFromPdf(file.fileUrl);
      
      // Validate if the resume was parsed correctly
      if (!resume || !resume.profile) {
        throw new Error("Failed to parse resume correctly");
      }
      ...
      if (getHasUsedAppBefore()) {
        ...
        const sectionToFormToShow: Record<ShowForm, boolean> = {
          workExperiences: resume.workExperiences && resume.workExperiences.length > 0,
          educations: resume.educations && resume.educations.length > 0,
      ...
      alert("There was an error parsing your resume. Please try a different PDF file.");
    }
  };
